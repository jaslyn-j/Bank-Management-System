...
public class RegisterFrame extends JFrame {
    ...
    public RegisterFrame(
            LoginFrame parentLoginFrame) {
        this.authService =
                new AuthService();
        this.selectedBranch =
                Session.getInstance()
                        .getSelectedBranch();
        this.parentLoginFrame =
                parentLoginFrame;
    ...
    private JPanel buildRegistrationCard() {
        ...
        JLabel cardSub = new JLabel(
                ...
                SwingConstants.CENTER);
        cardSub.setFont(new Font(
                "Segoe UI", Font.PLAIN, 11));
        cardSub.setForeground(
                UIConstants.TEXT_SECONDARY);
        cc.gridy = 1;
        cc.insets =
                new Insets(0, 20, 12, 20);
        card.add(cardSub, cc);
        ...
        // City and State row
        JPanel cityStateRow = new JPanel(
                new GridLayout(1, 2, 10, 0));
        ...
        stateField =
                UIUtils.createTextField(
                        "State");
        cityStateRow.add(buildSubField(
                "City *", cityField));
        cityStateRow.add(buildSubField(
                "State *", stateField));
        cc.gridy = 10;
        ...
        cc.gridy = 13;
        cc.insets =
                new Insets(0, 20, 4, 20);
        card.add(pwRow, cc);

        // Password hint
        JLabel pwHint = new JLabel(
                "Minimum 6 characters");
        pwHint.setFont(new Font(
                "Segoe UI", Font.ITALIC, 10));
        pwHint.setForeground(
        ...
        alreadyLabel.setFont(new Font(
                "Segoe UI", Font.PLAIN, 11));
        alreadyLabel.setForeground(
                UIConstants.TEXT_SECONDARY);
        ...
        SwingWorker<Boolean, Void> worker =
                new SwingWorker<>() {
                    ...
                    protected Boolean doInBackground() {
                        ...
                        // Set four address fields
                        customer.setStreet(street);
                        customer.setCity(city);
                        customer.setState(state);
                        customer.setPinCode(pinCode);

                        // Password is hashed inside
    ...
    // Sub field used inside GridLayout rows
    private JPanel buildSubField(
            String labelText,
            JComponent field) {
        JPanel panel = new JPanel(
                new BorderLayout(0, 4));
    ...
    // GridBagConstraints for section labels
    private GridBagConstraints sectionGbc(
            GridBagConstraints cc, int row) {
        GridBagConstraints g =
                (GridBagConstraints) cc.clone();
        g.gridy = row;
        g.insets = new Insets(
                10, 20, 4, 20);
        return g;
